...
for ix, d in enumerate(dists):
    ...
    driver = DMRGDriver(scratch='/central/scratch/hczhai/st-07-04', symm_type=SymmetryTypes.SU2, stack_mem=70 << 30, n_threads=24)
    ...
    bond_dims = [200] * 32
    noises = [1e-4] * 8 + [5e-5] * 8 + [1e-5] * 8 + [0]
    thrds = [1e-10] * 16 + [1e-11] * 16
    if ix == 0:
        ket = driver.get_random_mps(tag="GS", bond_dim=250, nroots=1)
    else:
        ket = ket0.deep_copy('GS')
        ket = driver.adjust_mps(ket, dot=2)[0]
    e0 = driver.dmrg(mpo, ket, n_sweeps=32, tol=1E-24, twosite_to_onesite=24,
        bond_dims=bond_dims, noises=noises, thrds=thrds, iprint=2)
    ket0 = ket.deep_copy('KET0')

    print('DMRG (M=200) energy = %20.15f' % e0)
