...
export PYSCF_TMPDIR=/central/scratch/hczhai/hife/fe2ocl6-def2-SVP-uks.1/runs/stdmrg-6
...
if [ "$?" = "1" ] || [ "${SLURM_TASKS_PER_NODE}" = "" ] || [ "0" = "1" ]; then
    if [ "0" = "1" ]; then
        ...
        ln -s /central/scratch/hczhai/hife/fe2ocl6-def2-SVP-uks.1/runs/stdmrg-6/FCIDUMP ./FCIDUMP
        cp /central/scratch/hczhai/hife/fe2ocl6-def2-SVP-uks.1/runs/stdmrg-6/${SCPT}.conf ${SCPT}.conf.${TJ}
        python3 -u $(which block2main) ${SCPT}.conf.${TJ} > ${SCPT}.out.${TJ}
    else
        python3 -u ${SCPT}.py 1 > ${SCPT}.out.${TJ}
    fi
else
    if [ "0" = "1" ]; then
        [ -f ./FCIDUMP ] && rm ./FCIDUMP
        ...
        if [ "$XRUN" = "srun" ]; then
            srun python3 -u ${SCPT}.py 1 > ${SCPT}.out.${TJ}
        else
            $XRUN --map-by ppr:$SLURM_TASKS_PER_NODE:node:pe=$OMP_NUM_THREADS \
                python3 -u ${SCPT}.py 1 > ${SCPT}.out.${TJ}
        fi
...
if [ "0" = "1" ]; then
    cp /central/scratch/hczhai/hife/fe2ocl6-def2-SVP-uks.1/runs/stdmrg-6/node0/1pdm.npy ${SCPT}.1pdm.${TJ}.npy
fi